...
construct_feature = 1  # determine whether construct extra features
testSet = 0  # test=1,test set; test=0,train set;
...
simplifysample1 = some_function.SimplifySample(600, imgs1)  # simplify sample by reducing features that are constant 0
...
if construct_feature == 1:  # for the case of constructing extra features
    if linear == 1:  # if using linear LS method
        ...
        random_mat = np.loadtxt("p_classifier2_2Matrix_11_29_11_22.txt")
        x = np.mat(np.loadtxt("p_classifier2_2_11_29_11_22.txt")).T
    ...
    if linear == 1:
        ...
        feature_mat = test_imgs
        x = np.mat(np.loadtxt("p_classifier2_1_11_28_19_49.txt")).T
